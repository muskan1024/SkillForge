...
const LANGUAGES = [
  ...
  { id: 'javascript', label: 'JavaScript',  judge0Id: 63,  color: 'bg-yellow-50 text-yellow-700 border-yellow-200', dot: 'bg-yellow-500', starter: '// JavaScript Playground\nconsole.log("Hello, SkillForge!");\n\n// Try arrays, functions, loops\nconst skills = ["HTML", "CSS", "JavaScript"];\nskills.forEach((skill, i) => {\n    console.log(`${i+1}. Learning ${skill}`);\n});\n' },
  ...
  { id: 'csharp',     label: 'C#',          judge0Id: 51,  color: 'bg-purple-50 text-purple-700 border-purple-200', dot: 'bg-purple-500', starter: 'using System;\n\nclass Program {\n    static void Main() {\n        Console.WriteLine("Hello, SkillForge!");\n        \n        string[] skills = {"C#", ".NET", "OOP"};\n        foreach (var s in skills) {\n            Console.WriteLine($"Learning: {s}");\n        }\n    }\n}\n' },
  ...
  { id: 'rust',       label: 'Rust',        judge0Id: 73,  color: 'bg-red-50 text-red-700 border-red-200',         dot: 'bg-red-500',    starter: 'fn main() {\n    println!("Hello, SkillForge!");\n    \n    let skills = vec!["Rust", "Ownership", "Borrowing"];\n    for (i, s) in skills.iter().enumerate() {\n        println!("{}. Learning {}", i+1, s);\n    }\n}\n' },
      ...
      <div className="bg-[#1e1e2e] px-4 py-2.5 flex items-center gap-2 border-b border-white/10">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button onClick={runCode} disabled={running}
            className="flex items-center gap-1.5 bg-green-500 hover:bg-green-400 disabled:opacity-50 text-white text-xs font-medium px-3 py-1.5 rounded-lg transition-colors">
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="bg-[#1e1e2e] px-4 py-2.5 flex items-center gap-2 border-b border-white/10">
        ...
        <button onClick={() => setPreview(html)} className="ml-auto flex items-center gap-1.5 bg-green-500 hover:bg-green-400 text-white text-xs font-medium px-3 py-1.5 rounded-lg transition-colors">
          <Send size={12}/> Run ▶
        </button>
      </div>
      <div className="flex flex-1 min-h-0">
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          <label className="block text-xs font-medium text-ink-secondary mb-1.5">Language</label>
        ...
        <div>
          <label className="block text-xs font-medium text-ink-secondary mb-1.5">Context (optional)</label>
          <input className="input text-sm" placeholder="e.g. Binary search implementation" value={ctx} onChange={e => setCtx(e.target.value)}/>
        </div>
      </div>
      <div>
        <label className="block text-xs font-medium text-ink-secondary mb-1.5">Your Code</label>
        <textarea rows={12} value={code} onChange={e => setCode(e.target.value)}
          className="w-full bg-[#1e1e2e] text-green-400 font-mono text-xs rounded-xl p-4 border border-surface-3 focus:outline-none focus:ring-2 focus:ring-brand-400 resize-none"
      ...
      {review && (
        <div className="p-5 bg-surface-1 rounded-xl border border-surface-3">
          <pre className="whitespace-pre-wrap text-sm text-ink-secondary font-sans leading-relaxed">{review}</pre>
        </div>
      )}
    </div>
  )
  ...
  return (
    <div className="animate-fade-in">
      <div className="mb-5">
        ...
        <p className="text-ink-tertiary mt-1 text-sm">Write, run, and review code — all inside SkillForge, no redirects</p>
      </div>
      ...
      <div className="flex gap-2 mb-5 flex-wrap">
        {tabs.map(t => (
          <button key={t.id} onClick={() => setActiveTab(t.id)}
            className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${activeTab===t.id ? 'bg-brand-600 text-white shadow-sm' : 'bg-white border border-surface-3 text-ink-secondary hover:border-brand-200'}`}>
            {t.label}
          </button>
        ))}
      </div>
          ...
          <div className="flex gap-2 mb-4 flex-wrap">
            {LANGUAGES.filter(l => l.id !== 'sql').map(l => (
              <button key={l.id} onClick={() => setActiveLang(l.id)}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium border transition-all ${activeLang===l.id ? `${l.color} border-current shadow-sm` : 'bg-white border-surface-3 text-ink-secondary hover:border-brand-200'}`}>
                <div className={`w-2 h-2 rounded-full ${l.dot}`}/>
                {l.label}
              </button>
            ))}
            <button onClick={() => setActiveLang('sql')}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium border transition-all ${activeLang==='sql' ? 'bg-teal-50 text-teal-700 border-teal-200' : 'bg-white border-surface-3 text-ink-secondary hover:border-brand-200'}`}>
              <div className="w-2 h-2 rounded-full bg-teal-500"/>SQL
            </button>
          </div>

          {/* IDE panel */}
          <div className="rounded-2xl overflow-hidden border border-surface-3 shadow-lg" style={{ height: '520px' }}>
            {activeLang === 'sql'
              ? <SQLEditor/>
      ...
      {activeTab === 'web' && (
        <div>
          <div className="rounded-2xl overflow-hidden border border-surface-3 shadow-lg" style={{ height: '540px' }}>
            <WebEditor/>
          </div>
          <p className="text-xs text-ink-ghost mt-3 text-center">Edit HTML/CSS/JS on the left and click Run to see live preview on the right</p>
        </div>
      )}
      ...
      {activeTab === 'review' && (
        <div className="card p-5 sm:p-6">
          ...
          <p className="text-sm text-ink-tertiary mb-5">Paste your code for instant AI feedback on bugs, style, and best practices</p>
          <CodeReview/>
        </div>
      )}
    </div>
