  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-surface-3 px-6 py-4 flex items-center justify-between rounded-t-2xl">
          <div>
            <h2 className="font-semibold text-ink-primary">Quiz: {topic.name}</h2>
            <p className="text-xs text-ink-ghost mt-0.5">Score 60%+ to auto-complete this topic</p>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-surface-2 rounded-xl text-ink-ghost"><X size={18}/></button>
        </div>
        ...
        <div className="p-6">
          {loading ? (
            <div className="flex flex-col items-center py-12">
              ...
              <p className="text-sm text-ink-tertiary">Generating quiz questions…</p>
            </div>
          ) : result ? (
            <div className="text-center py-8">
              ...
              <h3 className="text-2xl font-bold text-ink-primary mb-1">{result.score}%</h3>
              <p className="text-ink-tertiary mb-2">{result.correct}/{result.total} correct</p>
              <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium mb-6 ${result.passed ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-600'}`}>
              ...
              <div className="text-left space-y-4 mt-6">
                {questions.map((q, i) => {
                  ...
                  return (
                    <div key={i} className={`p-4 rounded-xl border ${isRight ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                      <p className="text-sm font-medium text-ink-primary mb-2">{i+1}. {q.question}</p>
                      <p className={`text-xs mb-1 ${isRight ? 'text-green-700' : 'text-red-600'}`}>
                        Your answer: {q.options[userAns] || 'Not answered'}
                      </p>
                      {!isRight && <p className="text-xs text-green-700">Correct: {q.options[correct]}</p>}
                      {q.explanation && <p className="text-xs text-ink-tertiary mt-2 italic">{q.explanation}</p>}
                    </div>
                  )
                })}
              </div>
            ...
            <div className="space-y-6">
              {questions.map((q, qi) => (
                <div key={qi} className="p-4 bg-surface-1 rounded-xl">
                  <p className="text-sm font-semibold text-ink-primary mb-3">{qi+1}. {q.question}</p>
                  <div className="space-y-2">
                    {q.options.map((opt, oi) => (
                      <label key={oi} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${answers[qi] === oi ? 'bg-brand-50 border-brand-400' : 'bg-white border-surface-3 hover:border-brand-200'}`}>
                        <input type="radio" name={`q${qi}`} className="hidden" onChange={() => setAnswers(p => ({...p, [qi]: oi}))}/>
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ${answers[qi] === oi ? 'border-brand-500 bg-brand-500' : 'border-surface-4'}`}>
    ...
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
        <div className="border-b border-surface-3 px-6 py-4 flex items-center justify-between">
          <div>
            <h2 className="font-semibold text-ink-primary">Study Notes: {topic.name}</h2>
            <p className="text-xs text-ink-ghost mt-0.5">AI-generated comprehensive notes</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (
            <div className="flex flex-col items-center py-16">
              <Loader2 size={32} className="text-brand-500 animate-spin mb-3"/>
              <p className="text-sm text-ink-tertiary">Generating study notes…</p>
            </div>
          ) : (
            <pre className="whitespace-pre-wrap text-sm text-ink-secondary font-sans leading-relaxed">{notes}</pre>
          )}
        </div>
      </div>
    </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <p className="text-xs font-semibold text-ink-ghost uppercase tracking-wide">My Notes</p>
        <span className={`text-[10px] ${saved ? 'text-green-500' : 'text-ink-ghost'}`}>{saved ? '✓ Saved' : 'Saving…'}</span>
      </div>
            ...
            <div key={topic.id} className="flex flex-col items-center">
              <div onClick={() => onTopicClick(topic)}
                className={`relative cursor-pointer w-56 sm:w-64 rounded-2xl border-2 p-4 transition-all hover:shadow-lg ${
                  ...
                  isCurrent ? 'bg-brand-50 border-brand-500 shadow-lg shadow-brand-100' :
                  'bg-white border-surface-3 hover:border-brand-300'
                }`}>
                {isCurrent && (
                ...
                <div className="flex items-start gap-3">
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 mt-0.5 ${
                    isDone ? 'bg-green-500 text-white' : isCurrent ? 'bg-brand-600 text-white' : 'bg-surface-3 text-ink-ghost'
                  }`}>
                    {isDone ? <Check size={13}/> : i + 1}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className={`text-xs font-semibold truncate ${isCurrent ? 'text-brand-700' : isDone ? 'text-green-700' : 'text-ink-primary'}`}>{topic.name}</p>
                    <p className="text-[10px] text-ink-ghost mt-0.5">Week {topic.week} · {topic.estimated_hours}h</p>
                  </div>
                </div>
              </div>
              {i < topics.length - 1 && (
                <div className={`w-0.5 h-6 ${topics[i+1]?.completed ? 'bg-green-400' : i < currentIndex ? 'bg-green-300' : 'bg-surface-3'}`}/>
              )}
            </div>
          )
        })}
...
function TopicCard({ topic, index, isCurrent, onQuiz, onAskAI, onNotes, roadmapId }) {
  ...
  return (
    <div id={`topic-${topic.id}`} className={`rounded-2xl border transition-all duration-200 ${
      isCurrent ? 'border-brand-400 shadow-md shadow-brand-100 bg-white' :
      topic.completed ? 'border-surface-3 bg-surface-1' : 'border-surface-3 bg-white'
    }`}>
      {isCurrent && (
      ...
      <div className="p-4 sm:p-5">
        <div className="flex items-start gap-3 sm:gap-4">
          <div className={`w-6 h-6 sm:w-7 sm:h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 mt-0.5 ${
            topic.completed ? 'bg-green-500 text-white' : isCurrent ? 'bg-brand-600 text-white' : 'bg-surface-3 text-ink-ghost'
          }`}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              <div>
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  ...
                  <span className={`badge text-[10px] ${DIFF_COLORS[topic.difficulty] || 'bg-surface-2 text-ink-tertiary'}`}>{topic.difficulty}</span>
                  {topic.quiz_score !== undefined && (
                    <span className="badge bg-green-50 text-green-700 text-[10px]">Quiz: {topic.quiz_score}%</span>
                  )}
                </div>
                <h3 className={`font-semibold text-sm sm:text-base ${topic.completed ? 'text-ink-ghost line-through' : isCurrent ? 'text-brand-700' : 'text-ink-primary'}`}>{topic.name}</h3>
              </div>
              <div className="flex items-center gap-1.5 shrink-0">
                <span className="flex items-center gap-1 text-xs text-ink-ghost"><Clock size={11}/>{topic.estimated_hours}h</span>
            ...
            {open && (
              <div className="mt-4 space-y-4 animate-slide-up">
                {topic.description && <p className="text-sm text-ink-tertiary leading-relaxed">{topic.description}</p>}
                ...
                <div className="flex flex-wrap gap-2 pt-1 border-t border-surface-3">
                  {!topic.completed && (
                  ...
                  <button onClick={() => onNotes(topic)}
                    className="flex items-center gap-1.5 px-3 py-2 bg-purple-50 hover:bg-purple-100 border border-purple-200 text-purple-700 rounded-xl text-xs font-medium transition-all">
                    <FileText size={13}/> Study Notes
                  </button>
                  <button onClick={() => onAskAI(topic)}
                    className="flex items-center gap-1.5 px-3 py-2 bg-brand-50 hover:bg-brand-100 border border-brand-200 text-brand-700 rounded-xl text-xs font-medium transition-all">
                    <Bot size={13}/> Ask AI
                  </button>
                </div>
              </div>
    ...
    <div className="max-w-3xl mx-auto animate-fade-in">
      ...
      <div className="card p-5 sm:p-6 mb-6">
        <div className="flex items-start justify-between gap-4 mb-4">
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-2">
              ...
              {roadmap.progress_percent >= 100 && <span className="badge bg-green-50 text-green-700">✓ Completed</span>}
            </div>
            <h1 className="text-lg sm:text-xl font-semibold text-ink-primary">{roadmap.title}</h1>
            <p className="text-sm text-ink-tertiary mt-1">Goal: {roadmap.learning_goal}</p>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <button onClick={exportPDF} title="Export" className="p-2 text-ink-ghost hover:text-brand-600 hover:bg-brand-50 rounded-xl transition-colors">
              <Download size={18}/>
            </button>
            <button onClick={handleDelete} disabled={deleting} className="p-2 text-ink-ghost hover:text-red-500 hover:bg-red-50 rounded-xl transition-colors">
              {deleting ? <Loader2 size={18} className="animate-spin"/> : <Trash2 size={18}/>}
            </button>
          </div>
        </div>
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm text-ink-secondary font-medium">{roadmap.completed_topics}/{roadmap.total_topics} topics</span>
        ...
        <div className="flex gap-2 mt-4">
          <button onClick={() => setViewMode('list')} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${viewMode==='list' ? 'bg-brand-600 text-white' : 'bg-surface-2 text-ink-secondary hover:bg-surface-3'}`}>List View</button>
          <button onClick={() => setViewMode('graph')} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${viewMode==='graph' ? 'bg-brand-600 text-white' : 'bg-surface-2 text-ink-secondary hover:bg-surface-3'}`}>Visual Map</button>
        </div>
      </div>
      ...
      {viewMode === 'graph' && (
        <div className="card p-6 mb-6">
          <h2 className="text-sm font-semibold text-ink-primary mb-6 text-center">Your Learning Journey</h2>
          <GraphicalRoadmap
      ...
      {roadmap.progress_percent >= 100 && (
        <div className="card p-8 text-center mt-4">
          ...
          <p className="text-sm text-ink-tertiary mb-4">You've mastered all topics. Ready for the next challenge?</p>
